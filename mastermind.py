...
s = solutions()
# Create a copy swg of the solutions set s
swg=s.copy()

# Pick a random secret code
secret = choice (s)
print ("Code:  ",secret, "| Black | White | Codes remaining |")
...
while len(s)>1:
    b=0
    w=0
# The Knutt routine requires a working copy of all remaining non used quesses, removing the guess from the set after each guess.
    swg.remove(guess)
...
# Print the last guess, number of Blacks and Whites and remaining potential solutions    
...
# Update number of guesses
    guesses+=1
# Using the Knutt routine to optimize the next guess    
    guess = minmax (swg, s)
# Present the found code and number of total guesses    
    if len(s)==1:
        print ("Secret:", s[0],"found in",guesses, end="")
# Print alignment for one or multiple guesses
        if guesses==1: print (" guess")
        else:          print (" guesses")