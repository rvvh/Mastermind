# Mastermind 
# 07JUN20 Cracking a Length 4 code with 6 [1-6] Options with Duplicates allowed (s = 1296 options)
...
# Mods 25APR26 Layout improvements and alignment
...
def solutions():
    s = np.int_(np.loadtxt('s.txt', delimiter=","))
    return (s)
...
# Optimizes the next guess to assure a win in 5 guesses or less.
# May even choose a guess guaranteed not to win, but is necessary to finish in 5 guesses
def minmax (swg,s):
    t=np.zeros(len(swg))
...
# Get all possible solution combinations
s = solutions()
scores = []
# Make copy swg of solutions set s
swg=s.copy()

# Pick a random secret code
secret = choice (s)
print ("┌───┬───────────┬───────┬───────┬───────────┐")
print ("│ # │   Guess   │ Black │ White │ Remaining │")
print ("├───┼───────────┼───────┼───────┼───────────┤")
...
guess   = np.array([1, 1, 2, 2], dtype=np.int64)
guesses = 1

while len(s)>1:
    b=0
    w=0
# Knutt routine keeps a working copy of non-used quesses, removing the guess from the set after each guess.
    np.delete(swg, guess)
...
# Print last guess, number of Blacks and Whites and remaining potential solutions    
    print ("│",guesses,"│",guess,"│  ",black,"  │  ",white, "  │",end=" ")
...
# Present code and number of total guesses    
    if (guess==secret).all():
        ...
        print ("│",guesses,"│",guess,"│   4   │   0   │     0     │",)
        print ("└───┴───────────┴───────┴───────┴───────────┘")
        print ("Solved in",'{:.1f}'.format(toc - tic),"seconds and",guesses,"guess",end="")
